...
{searchOpen && (
  <div
    className="fixed inset-x-0 top-[130px] bg-softwhite border-b border-darktext/10 z-50"
    onClick={() => {
  ...
  >
    ...
    >
      ...
      >
        ...
        <button type="submit">
          <Search className="w-6 h-6 text-darktext hover:text-primary" />
        </button>
      </form>

    {/* SUGGESTIONS */}
{suggestions.length > 0 && (
  <ul className="mt-3 border-t pt-2 max-h-60 overflow-y-auto animate-fadeSlide">
    {suggestions.map((p, index) => (
      <li
        key={p._id}
        className="py-3 px-2 cursor-pointer hover:bg-gray-100 transition-all duration-200 rounded-md opacity-0 animate-fadeItem"
